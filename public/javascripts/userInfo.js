...
async function saveUserInfo(){
    ...
    let about = document.getElementById("userBioInput").value;
    try {
        ...
        document.getElementById("aboutStatus").innerText = "Error";
        throw error;
    }
    document.getElementById("userBioInput").value = "";
    loadUserInfo();
}
...
async function loadUserInfo(){
    ...
    let infoJson = await fetchJSON(`api/${apiVersion}/users/userInfo?username=${username}`);
    let infoHtml = infoJson.map((info) => {
        return `
        <p>${info.userBio}</p>
        `;
    });
    document.getElementById("user_info_div").innerHTML = infoHtml;
